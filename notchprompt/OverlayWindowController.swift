...
private final class OverlayPanel: NSPanel {
    ...
    @objc func clearScript(_ sender: Any?) {
        Task { @MainActor in
            PrompterModel.shared.clearScript()
        }
    }
    ...
    override func rightMouseDown(with event: NSEvent) {
        ...
        let menu = NSMenu()
        menu.addItem(NSMenuItem(title: L(.ctxPaste), action: #selector(paste(_:)), keyEquivalent: ""))
        menu.addItem(NSMenuItem(title: L(.ctxClear), action: #selector(clearScript(_:)), keyEquivalent: ""))
...
final class OverlayWindowController {
    ...
    func reposition() {
        ...
        // Grow the notch while voice-following (or in 随讲/cue mode) for read-ahead room.
        let desiredHeight = NotchLayout.height(base: model.overlayHeight,
                                               voiceActive: VoiceFollowController.shared.isListening,
