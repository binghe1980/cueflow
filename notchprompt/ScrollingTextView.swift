...
struct ScrollingTextView: View {
    ...
    private var emptyStateMessage: String {
        LocalizationManager.shared.l(.scrollEmpty)
    }
        ...
        GeometryReader { viewportProxy in
            TimelineView(.periodic(from: .now, by: tickInterval)) { timeline in
                ZStack(alignment: .topLeading) {
                    if hasContent {
                        // Render the script whenever there is content — even before
                        // the session starts — so pasted/typed/imported text is
                        // immediately visible (static at the top). Scrolling only
                        // begins on Start. Repeated copies keep mode toggles cheap.
